...
- `isPresentation`, `reportFileR2Key` 모두 null/미전송이면 400
- `reportFileR2Key` 변경 시 `r2FileUploadId`도 필수

### 파일 검증 흐름 (`reportFileR2Key` 변경 시)
...
| `r2_file_upload`        | 파일 검증 + `isVerified` UPDATE (`reportFileR2Key` 변경 시) |
...
| `reportFileR2Key`     | string (nullable)  | 변경할 파일의 R2 key               |
| `r2FileUploadId` | id (nullable)      | `reportFileR2Key` 변경 시 필수          |

**알림 규칙**:

- `isPresentation`만 변경 → 그룹 멤버 전체 + 운영진에게 발표 여부 변경 알림
- `reportFileR2Key`만 변경 → 그룹 멤버 전체 + 운영진에게 제출 파일 변경 알림
...
| `reportFileR2Key`     | string    | 보고 파일 R2 key                |
...
4. `isPresentation`, `reportFileR2Key` 모두 null/미전송 → 400
5. `reportFileR2Key` 전송 시 `r2FileUploadId` 미전송 → 400
6. `isPresentation`만 전송 → `isPresentation` 업데이트, 200 반환
7. `reportFileR2Key`만 전송 → 파일 검증 통과 시 기존 R2 파일 삭제 + `reportFileR2Key` 업데이트, 200 반환
8. 둘 다 전송 → 파일 검증 통과 시 기존 R2 파일 삭제 + 둘 다 업데이트, 200 반환

- 파일 검증 관련 (`reportFileR2Key` 변경 시)
...
- 알림 관련
  ...
  2. `reportFileR2Key`만 변경 시 → 그룹 멤버 전체 + 운영진에게 제출 파일 변경 알림
