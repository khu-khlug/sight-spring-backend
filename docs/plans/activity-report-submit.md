...
### 파일 검증 흐름 (`r2FileUploadId` + `reportFile`)
...
7. 모두 통과 → `isVerified = true` 설정 후 진행
...
| `reportFile`      | string  | R2 object key (파일 하나만, 여러 개면 압축)       |
...
4. 요청 바디에 `reportFile` 또는 `r2FileUploadId` 없음 → 400
5. 그룹원(비그룹장) 요청 → 403
