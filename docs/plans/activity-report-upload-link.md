...
1. **key 용도 검증 불가**: 활동보고 제출 시 `reportFileR2Key`가 실제로 활동보고용으로 발급된 것인지 확인할 수 없다. 다른 API에서 발급받은 key를 가져다 쓰는 것을 막을 방법이 없다.
...
| `r2_file_upload` | 발급된 key INSERT (`id`, `key`, `memberId`, `api_path`, `isVerified`, `created_at`) |
