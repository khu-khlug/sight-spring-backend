...
그룹 목록을 다양한 조건으로 필터링하여 조회합니다. 타입별(프로그램/연구), 상태별(진행 중/성공), 관심 분야별 필터와 키워드 검색을 지원하며, 기존의 참여 여부·즐겨찾기 필터와 조합하여 사용할 수 있습니다.
...
| `type`       | 문자열 | 그룹 타입 필터. `program` (교육·프로그램·운영) 또는 `research` (스터디·프로젝트·문서화) |
...
| `orderBy`    | 문자열 | 정렬 기준. `changedAt` (최근 활동순) 또는 `createdAt` (생성순, 기본값)                 |
...
1. `type=program`으로 조회하면 교육·프로그램·운영 카테고리 그룹만 반환한다
   - `education`, `program`, `manage` 카테고리 그룹만 결과에 포함되어야 한다
2. `type=research`로 조회하면 스터디·프로젝트·문서화 카테고리 그룹만 반환한다
   - `study`, `project`, `documentation` 카테고리 그룹만 결과에 포함되어야 한다
3. `type` 파라미터에 유효하지 않은 값을 전달하면 400 에러를 반환한다
...
15. 여러 필터를 동시에 사용할 수 있다 (AND 조합)
    - `type=research&state=progress&keyword=Spring` 같은 조합이 정상 동작해야 한다
